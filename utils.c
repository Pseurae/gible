...
// Standard 256 LUT CRC32
inline void crc32_adjust(uint32_t *crc32, uint8_t input)
{
	*crc32 = ((*crc32 >> 8) & 0x00ffffff) ^ crc32_table[(*crc32 ^ input) & 0xff];
}
...
inline uint32_t crc32_calculate(const uint8_t *data, unsigned length)
{
	uint32_t crc32 = ~0;
	for (;length--;)
		crc32_adjust(&crc32, *(data++));

	return ~crc32;
}

inline void crc32_populate(const uint8_t *data, unsigned length, uint32_t *crc)
{
	*crc = crc32_calculate(data, length);
}